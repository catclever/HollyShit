# =========================================================================
# [ARCHIVED] LEGACY POINT-BASED FLOW MATCHER
# 
# [Original Design Logic]:
# This Flow Matcher was originally designed to predict a SINGLE isolated vector Z (1024-d).
# The idea was that Mamba would perform sentence-level macro reasoning and output a 
# single continuous coordinate. This Flow Matcher would then learn the Continuous 
# Normalizing Flow (solving an ODE) to map a standard Gaussian noise point x_0 
# to that deterministic semantic target Z_truth (x_1).
#
# [Reason for Archival]:
# It has been archived because a single pooled 1024-d vector loses precise sequence 
# length and positional ordering information, making it impossible to decode cleanly 
# via Non-Autoregressive (NAR) parallel generation.
# The architecture is migrating to Conditional Sequence Flow Matching, which predicts 
# a sequence of vectors [L, D] instead of a single point.
# =========================================================================
...
class AdaLN(nn.Module):
    ...
    def __init__(self, d_model: int, cond_dim: int):
        ...
        self.cond_proj.weight = mx.zeros(self.cond_proj.weight.shape)
        if self.cond_proj.bias is not None:
            self.cond_proj.bias = mx.zeros(self.cond_proj.bias.shape)
...
class FlowMatcher(nn.Module):
    ...
    def __init__(self, d_model: int, hidden_dim: int = 2048):
        ...
        self.out_proj.weight = mx.zeros(self.out_proj.weight.shape)
        if self.out_proj.bias is not None:
            self.out_proj.bias = mx.zeros(self.out_proj.bias.shape)
        
    def __call__(self, x_t: mx.array, t: mx.array, h_context: mx.array):
        """
        前向预测瞬时速度场。
        ...
        cond = mx.concatenate([t_emb, h_context], axis=-1)
    ...
    def predict_with_cfg(self, x_t: mx.array, t: mx.array, h_context: mx.array, cfg_scale: float = 3.0):
        """
        ...
        # 计算无条件的预测 (全零上下文)
        h_null = mx.zeros_like(h_context)
